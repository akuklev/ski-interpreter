class Expr {
  ...
  /**
   * Replace all instances of given free vars with their corresponding values
   * and return the resulting expression.
   * @param {[replace: FreeVar, withValue: Expr][]} pairs
   * @return {Expr}
   */
  subst (pairs) {
    return this.substRec(pairs) ?? this;
  }

  /**
   * subst() internal implementation.
   * Returns altered expression if there were changes or null if there were none.
  ...
  * walk (options = {}) {
  ...
  /**
   * Create an alias. Shorthand for new Alias(name, this).
   * @param {string} name
   * @return {Alias}
   */
  alias (name) {
...
class App extends Expr {
  ...
  substRec (list) {
    const fun = this.fun.substRec(list);
    let change = fun === null ? 0 : 1;
    const args = [];
    for (const x of this.args) {
      const next = x.substRec(list);
      if (next === null)
        args.push(x);
      else {
        args.push(next);
...
class Lambda extends Expr {
  ...
  constructor (args, impl) {
    ...
    this.impl = impl.substRec([[my, rename]]) ?? impl;
    this.args = [rename];
  }
  ...
  reduce (input) {
    ...
    return (this.impl.substRec(zip) ?? this.impl).apply(...input.slice(this.args.length));
  }

  substRec (list) {
    const change = this.impl.substRec(list);
    if (change)
...
class Alias extends Named {
  ...
  substRec (list) {
    return this.impl.substRec(list);
  }
